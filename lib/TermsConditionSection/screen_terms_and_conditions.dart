...
import 'package:careno/models/terms_and_conditons.dart';
...
class ScreenTermsAndConditions extends StatelessWidget {
  ScreenTermsAndConditions({Key? key}) : super(key: key);
    ...
    return SafeArea(
      child: Scaffold(
        ...
        body: StreamBuilder<QuerySnapshot>(
            stream: termsAndConditionsCollection.snapshots(),
            builder: (context, snapshot) {
              if(snapshot.connectionState == ConnectionState.waiting){
                return Center(child: CircularProgressIndicator.adaptive(backgroundColor: AppColors.appPrimaryColor,),);
              }
              var policys = snapshot.data!.docs.map((e) => TermsAndConditons.fromMap(e.data() as Map<String,dynamic>)).toList();
